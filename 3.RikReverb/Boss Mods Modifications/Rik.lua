...
-- local amplifierMarker = mod:AddMarkerOption(false, "npc", 1, -31087, 8, 7, 6, 5, 4, 3, 2, 1)
...
function mod:OnEngage()
    ...
    if not self:Easy() then
        self:Bar(472306, timers[472306][sparkblastIgnitionCount],
            CL.count:format(L.sparkblast_ignition, fullSparkblastIgnitionCount))                                   -- Sparkblast Ignition
    end
...
function mod:Amplification(args)
    ...
    self:CDBar(args.spellId, timers[args.spellId][amplificationCount],
        CL.count:format(L.amplification, fullAmplificationCount))
end
...
function mod:SoundCannon(args)
    ...
    self:Bar(args.spellId, timers[args.spellId][soundCannonCount + 1],
        CL.count:format(args.spellName, fullSoundCannonCount + 1))
end
...
do
    ...
    function mod:PyrotechnicsSpawn(args)
        if args.time - prev > 3 then
            ...
            self:Bar(472306, timers[472306][sparkblastIgnitionCount],
                CL.count:format(L.sparkblast_ignition, fullSparkblastIgnitionCount))
        end
    end
end
